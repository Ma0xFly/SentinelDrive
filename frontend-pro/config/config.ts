...
export default defineConfig({
  ...
  //================ pro 插件配置 =================
  /**
   * @name openAPI 插件：从后端 OpenAPI schema 生成服务层。
   * 当前服务层为手写（src/services/，结构与本插件产物一致），
   * 待后端 schema 可导出后，生成 openapi.json 并取消注释即可切换。
   * schema 导出：python -c "from app.main import app; import json; print(json.dumps(app.openapi()))" > openapi.json
